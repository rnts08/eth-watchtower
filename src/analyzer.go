...
var (
	...
	selectors = map[[4]byte]string{
		{0x40, 0xc1, 0x0f, 0x19}: "Mintable",
		{0x42, 0x96, 0x6c, 0x68}: "Burnable",
		{0xf2, 0xfd, 0xe3, 0x8b}: "Ownable",
		{0x1d, 0x3b, 0x9e, 0xdf}: "Blacklist",
		{0xfe, 0x57, 0x5a, 0x87}: "Blacklist",
		{0x36, 0x59, 0xcf, 0xe6}: "Upgradable",
		{0x01, 0xff, 0xc9, 0xa7}: "InterfaceCheck",
		{0x67, 0x34, 0x48, 0xdd}: "IncorrectConstructor",
		{0x3c, 0xcf, 0xd6, 0x0b}: "Withdrawal",
		{0x2e, 0x1a, 0x7d, 0x4d}: "Withdrawal",
		{0x71, 0x50, 0x18, 0xa6}: "RenounceOwnership",
		{0x5c, 0xff, 0xe9, 0xde}: "FlashLoan",
		{0x81, 0x29, 0xfc, 0x1c}: "ReinitializableProxy",
		{0x09, 0x5e, 0xa7, 0xb3}: "ApprovalFunction",
		{0x10, 0xd1, 0xe8, 0x5c}: "FlashLoanReceiver",
		{0x23, 0xe3, 0x0c, 0x8b}: "FlashLoanReceiver",
	}
)
